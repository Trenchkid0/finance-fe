...
export function SkeletonDashboard() {
  return (
    <div className="space-y-4 pb-8">
      ...
      {/* ROW 1 — Net Worth (2/3) + Asset Summary (1/3) */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {/* NetWorthHero skeleton */}
        <div className="lg:col-span-2 rounded-xl border border-border bg-card overflow-hidden">
          <div className="flex items-start justify-between gap-4 p-5 pb-3">
            ...
            <div className="flex gap-1.5">
              {[1,2,3,4,5,6].map(i => <Skeleton key={i} className="h-7 w-10 rounded-full" />)}
            </div>
          </div>
          <div className="h-52 px-4 pb-3">
            <Skeleton className="h-full w-full rounded-lg" />
          </div>
        </div>

        {/* AssetSummaryWidget skeleton */}
        <div className="lg:col-span-1 rounded-xl border border-border bg-card p-6 flex flex-col gap-5">
        ...
        {[1,2,3,4].map(i => (
          <div key={i} className="rounded-xl border border-border bg-card p-5 flex flex-col gap-3">
            <div className="flex items-start justify-between gap-3">
              ...
              {/* DotMatrix placeholder */}
              <div className="grid grid-cols-4 gap-1 shrink-0">
                {Array.from({ length: 28 }).map((_, j) => (
                  <Skeleton key={j} className="size-1.5 rounded-full" />
                ))}
              </div>
            </div>
          </div>
      ...
      {/* ROW 3 — Tabs (chart) + Recent Transactions */}
